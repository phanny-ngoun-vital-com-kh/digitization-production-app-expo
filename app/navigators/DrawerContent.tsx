...
const Drawers: React.FC<DrawersProps> = ({ navigation, username, ...props }) => {
  ...
  const createTwoButtonAlert = () =>
    Alert.alert("ចាកចេញ", "តើ​អ្នក​ចង់​ចេញមែនទេ?", [
      {
        text: "ទេ",
    ...
    <View style={{ flex: 1 }}>
      <DrawerContentScrollView {...props}>
        <View>
          <View style={styles.userInfoSection}>
            <View style={{ flexDirection: "row", marginTop: 15 }}>
              <Avatar.Image
                source={require("../../assets/images/770137_man_512x512.png")}
                size={50}
              />
        ...
        <Drawer.Section style={{ marginTop: 10 }}>
          <Drawer.Item
            icon={({ color, size }) => <Icon name="home" color={color} size={size}></Icon>}
            style={{}}
            label="Home"
            onPress={() => {
              {
          ...
          ></Drawer.Item>
          <Drawer.Item
            icon={({ color, size }) => <Icon name="settings" color={color} size={size}></Icon>}
            label="Setting"
            onPress={() => {
              {
              }
      ...
      <Drawer.Section style={styles.bottomDrawerSection}>
        <DrawerItem
          icon={({ color, size }) => <Icon name="logout" color={color} size={size}></Icon>}
          label="ចាកចេញ"
